...
def review(employer_id):
    if request.method == 'POST':
        ...
        execute_sql('INSERT INTO review (review, rating, title, status, date, employer_id) VALUES (?, ?, ?, ?, ?, ?)', (review, rating, title, status, date, employer_id), commit=True)
        return redirect(url_for('employer', employer_id=employer_id))
        # return render_template('review_submitted.html', employer_id=employer_id)
